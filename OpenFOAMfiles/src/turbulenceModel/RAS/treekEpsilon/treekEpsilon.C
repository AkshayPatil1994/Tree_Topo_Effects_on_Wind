...
void treekEpsilon<BasicTurbulenceModel>::correct()
{
    ...
    (
     ...
     ==
      ...
      + fvm::Sp(plantCd_()*leafAreaDensity_()/k_()*(C4_*betaP_*pow(mag(U()),3)- C5_*betaD_*k_()*mag(U())), epsilon_) //Source Term
      //+ epsilonSource()
      //+ fvOptions(alpha, rho, epsilon_)
    );
    ...
    (
     ...
     ==
      ...
      + fvm::Sp(plantCd_() *leafAreaDensity_() / k_()*(betaP_ * pow(mag(U()),3)- betaD_ * k_() * mag(U())), k_)//Source Term
      + kSource()
